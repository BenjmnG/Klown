  ...
  for (int y = crop; y < height - crop;y = y + 2) {
    for (int x = 0; x < width;x = x + space) { 
      ...
      if(brightness > lim1t){
        ...
        if(currG > lim1t){
          fill(map(currG, lim1t, 255, 100, 130), 80, 100);
          ellipse(x + daz - space / 2, y + daz, pixel, pixel);
        }          
        if(currB > lim1t){
          fill(map(currB, lim1t, 255, 220, 250), 80, 100);
          ellipse(x + daz - space, y + daz, pixel, pixel);
        }
      }
    }
  }
