...
void draw() { 
  ...
  if(frameCount%1 == 0){ bg = !bg;}
  if(bg){background(#430e4a);} else {background(#124a0d);}
  //println(video.time());
  ...
  if(frameIndex > 30 && frameIndex < 45 || frameIndex > 60 && frameIndex < 70){
    maxDaz = maxDaz + space*12; 
    bri = map(frameCount, 30, 45, 0, 100);
  } else { 
    maxDaz = space * 10; 
    bri = 0;
  }
 ...
 void drawdot(){
  for (int y = crop; y < height - crop;y = y + 2) {
    for (int x = 0; x < width;x = x + space) { 
      ...
      if(brightness > lim1t){
        float pixel = map(brightness, lim1t, 255, maxWeight, 0);
        ...
        hue = map(brightness, lim1t, 100, 0, 360);
