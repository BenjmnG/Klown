...
float maxDaz = space*25;
float lim1t = 0; // seuil de luminosité à partir duquel image restranscrite
float unlim1ted = 30; // seuil de luminosité jusqu'auquel image restranscrite
float frameIndex = 0;
float startAt = 20;
float endAt; 
float last;
  ...
  for (int y = crop; y < height - crop;y = y + 2) {
    for (int x = 0; x < width;x = x + space) { 
      ...
      if(brightness > lim1t && brightness < unlim1ted){
        ...
        float daz = map(brightness, lim1t, unlim1ted, maxDaz, 0); // destroy the image with dazzling param
