...
void setup() {
  ...
  noStroke();
  frameRate(fps);
  video = new Movie(this, "../../../src/montage_graphique.mp4");
  video.volume(0);
  video.pause();
...
void draw() { 
  ...
  lim1t = 0; unlim1ted = 75;

  
    
  push();
  blendMode(DARKEST); //LIGHTEST / REPLACE
  drawdot(); filter(BLUR, 6);
  drawdot();
  pop();
  
  push();fill(0); text("VideoTime " + video.time(), 20,40); text("Framecount " + frameCount,20,60); pop();
    ...
    for (int x = 0; x < width;x = x + space) { 
      ...
      if(brightness > lim1t && brightness < unlim1ted){
        ...
        if(brightness < unlim1ted*f){
          pixel = map(brightness, lim1t, unlim1ted*f, maxWeight*.75, 0);
          _sat = map(brightness, lim1t, unlim1ted*f, sat, sat*.75);
        } else {
          pixel = map(brightness, unlim1ted*f, unlim1ted, maxWeight*.25, 0);
